...
request description name no record id and no `FAKE-REQ`, and MUST fail one
...
or a dependency update, names `FAKE-REQ` in place of a record id. `vogon init`
writes `vogon/fake/FAKE-REQ.md`, which says when to use it.
...
"FAKE-REQ — fix a typo in the installation guide", clears it. Naming
`REQ-TRK-99`, which does not exist, fails.

**Acceptance.** For any change under review, at least one record id or
`FAKE-REQ` appears in its commit messages or its description, and every id that
