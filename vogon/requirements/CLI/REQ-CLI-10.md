...
**Requirement.** When a Claude Code session starts in a project that has no
...
setup. The project is the directory in `CLAUDE_PROJECT_DIR`. Every other hook
MUST print nothing in that project until `vogon.yaml` exists.

The plugin is enabled only in projects that use VOGON (`REQ-CLI-9`), so a
session in which the hooks run is in such a project, and a missing
...
`<project>/vogon.yaml` does not exist", followed by the instruction to load
...
`vogon init`, and `vogon hook commit` prints nothing. Given a
