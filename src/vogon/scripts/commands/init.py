...
- the records directory with `modules.yaml`, `fake/FAKE-REQ.md` and the
  directories for each kind of record, held sources, plans and documents;
- `.vogon/` in `.gitignore`;
...
FAKE_REQ_FILE = f"{ids.FAKE_REQ_DIR}/{ids.FAKE_REQ}.md"
FAKE_REQ_TEXT = """# FAKE-REQ — Changes that need no requirement
...
def run(args, config: Config) -> list[Finding]:
    ...
    fake_req = config.records_dir / FAKE_REQ_FILE
    if not fake_req.exists():
        _write(fake_req, FAKE_REQ_TEXT, done, config)
